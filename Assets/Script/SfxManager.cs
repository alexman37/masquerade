...
public class SfxManager : MonoBehaviour
{
    ...
    // Play an audio clip specified in this manager's local list
    public void playSFXbyNameWithCutoff(string clipName, Transform spawnHere, float volumeLevel, float stopAfterTime)
    {
        AudioSource audioSource;
        if (spawnHere != null)
            audioSource = Instantiate(soundFXObject, spawnHere.position, Quaternion.identity);
        ...
        audioSource.volume = volumeLevel * globalSFXVolume;
        audioSource.Play();
        float clipLength = Mathf.Min(audioSource.clip.length, stopAfterTime);
